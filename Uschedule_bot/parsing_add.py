                ...
                for _i in numbers:
                    if len(_i) > 2:
                        ...
                        for _item in range(int(_i[0]), int(_i[1]) + 1):
                            numbers_of_weeks.append(_item + 21)
                    else:
                        numbers_of_weeks.append(int(_i) + 21)
            else:
                numbers_of_weeks.append(int(numbers) + 21)
    ...
    def transform_to_days(massive):
        ...
        for lesson in massive.get(day):
            ...
            for part in list_of_numbers_of_weeks:
                ...
                if lesson[1] != " ":
                    #  set the date of day
                    date = datetime.strptime((weeks.get(part)[:6]).rstrip(), "%d.%m.") + timedelta(days=d_of_week.get(day))
